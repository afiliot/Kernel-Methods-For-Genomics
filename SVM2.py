...
class SVM2():
    def __init__(self, K, ID, lbda=1e-2, maxiter=100, tol=1e-4, eps=1e-4, method='SLSQP', print_callbacks=True):
        self.K = K
        self.ID = ID
        self.lbda = lbda
    ...
    def cv(self, Ls, data, kfolds=5, pickleName='cv_2SVM'):
        ...
        for k in tqdm(range(kfolds)):
            ...
            s_tr, s_te = [], []
            for l in Ls:
                svm = SVM2(self.K, self.ID, lbda=l, print_callbacks=True); svm.fit(X_train, y_train)
                ...
                print('Lambda={}, accuracy on train ({:0.4f}) and val ({:0.4f})'.format(l, score_tr, score_te))
